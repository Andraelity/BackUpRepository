Shader "Shaders2D/PulsatingPink"
{
	Properties
	{
...
float3 palette( float t){
    ...
    float2 origine = uv;
    float3 finalColor = float3(0.6, 0.6, 0.6);
    
    for (float i = 0.0; i < 3.0; i++) {
    
        uv = frac(uv * 1.3) - 0.5;
    
        float d = length(uv) * exp(-length(origine));

        float3 col = palette(length(origine) + TIME * 0.4) + i * 0.4 + float3(-0.7, -1.3, d);
    
        d = cos(d*4. + TIME * 0.9)/8.0 / sin(d/4. + TIME)/8.0;
        d = abs(d);
