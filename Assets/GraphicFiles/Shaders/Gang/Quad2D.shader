...
float3 sdgQuad( in float2 p, in float2 v[4] )
{
    ...
    return float3(d,res.yz/d);
        ...
        float time = TIME;
        float2 v[4] = {
        float2(-0.9,-0.5) + 0.3*cos( 0.5*time + float2(0.0,1.9) + 4.0 ),
        float2( 0.9,-0.5) + 0.3*cos( 0.7*time + float2(0.0,1.7) + 2.0 ),
        float2( 0.9, 0.5) + 0.3*cos( 0.9*time + float2(0.0,1.3) + 1.0 ),
        float2(-0.9, 0.5) + 0.3*cos( 1.1*time + float2(0.0,1.5) + 0.0 ) };
